<template>
  <nav class="d-xs-flex">
    ...
    <transition name="slide-left">
      <div
        v-if="openSidebarMenu"
        class="mobilemenu_container z-xs-10 top-0 left-0 fixed h-100 bg-cl-sdg-lightgray w-80p"
      >
        <div class="d-xs-flex flex-xs-column">
          <div
            class="signin_container bg-cl-sdg-darkblue uppercase cl-white px-xs-20 h-xs-50 h-md-auto d-xs-flex align-item-center"
            @click="login"
          >
            <div class="user_icon relative mr12">
              <Icon class="white w-xs-24 h-xs-24" icon-id="login" />
              <i v-if="currentUser" class="logged-in absolute bg-cl-sdg-orange" />
            </div>
            <a class="link-white" v-if="!currentUser" href="#" @click.prevent="closeMenu">
              Sign In
            </a>
            <span v-if="currentUser" class="uppercase">
          ...
          <div class="menulinks_container font-bold">
            <MobileMenu
              ...
              :sharafexperiences="menuitems.sharafExperiences"
            />
            <div
              class="myaccountlinks_wrapper px-xs-20 py-xs-10 bg-cl-white"
              v-if="currentUser"
            >
          ...
          <div
            class="logout_country_container bg-cl-white brdr-top-1 brdr-cl-secondary"
          >
            <div class="d-xs-flex justify-space-around w-100 align-item-center">
              ...
              <a
                v-if="currentUser"
                href="#"
                class="w-50 d-xs-flex justify-center brdr-left-1 brdr-cl-secondary p-xs-20"
